...
export const LoginForm = () => {
  ...
  const { signIn, signInWithGoogle, resetPassword, currentUser } = useAuth();
  const router = useRouter();

  // Check if there are local quotes when user successfully logs in
  useEffect(() => {
    if (currentUser) {
      if (hasLocalQuotes()) {
        setShowMigration(true);
      } else {
        // If no quotes to migrate, redirect immediately
        router.push('/');
      }
    }
  }, [currentUser, router]);
